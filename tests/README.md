...
Its explorer checks exist because `cnProcessExplorer()` is the one place that
reads `ULSDBase::devname`, and nothing had ever covered it. `devname` was not
in the `ULSDBase` constructor's initializer list at all, so a device that did
not assign it - which is every subclass in the tree, including the `ULSD_PC`
the Qt wrapper hands to both of its transports - answered an explorer by
copying 16 bytes through an indeterminate pointer. It stayed invisible for as
long as the surrounding storage happened to hold a readable address. The
padding check is the second half of the same bug: the copy was a fixed 16
bytes out of a NUL-terminated string, so any name shorter than the field read
past the end of its own literal, and the wrapper builds a `QString` straight
off that field.
