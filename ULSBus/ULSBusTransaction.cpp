...
bool ULSBusTransaction::frameBufferCheck()
{
    ...
    if(_buf->isBufferComlite())
    {
        ...
        if(rez == ULSBUS_OBJECT_FIND_OK){
            ...
            ULSBUS_LOG("%s: OBJECT [0x%X] Received : self_id: 0x%X remote_id: 0x%X ",_connection->interface()->name(),_buf->id(),_self_id,_remote_id);
            if(!obj){
                ULSBUS_ERROR("%s: OBJECT [0x%X] Notfound (WTF) : self_id: 0x%X remote_id: 0x%X ",_connection->interface()->name(),_buf->id(),_self_id,_remote_id);
                close();
                return false; // WTF ?
            }
