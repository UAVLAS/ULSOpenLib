    ...
    if((_buf->size() + 2) <= _connection->maxFrameSize()){
        ...
        if(_buf->getData(_connection->maxFrameSize(),0,pxTxPack->aoi_sft.data,_buf->size())){ // If buffer ready
            ...
            if(_connection->send())
            {
                ...
                ULSBUS_ERROR("%s: Sending ULSBUS_AOI_SFT Connection failure : self_id: 0x%X remote_id: 0x%X ",_connection->interface()->name(),_self_id,_remote_id);
            }
        }else{
            //int i =10;
            _state = ULSBUST_AOI_TRANSMIT_START_REPEAT;
        ...
        if(_connection->send()){
            ...
            // Error interface buffer full repeat later
            ULSBUS_ERROR("%s: Sending ULSBUS_AOI_SOT FAIL : self_id: 0x%X remote_id: 0x%X ",_connection->interface()->name(),_self_id,_remote_id);
            _state = ULSBUST_AOI_TRANSMIT_START_REPEAT;
        }
    }
    return false;
...
bool ULSBusTransaction::frameTransmit(uint8_t *buf)
{
    ...
    if(_connection->interface()->send())
    {
        ...
        // IDLE
        ULSBUS_ERROR("%s: Sending FRAME FAIL : self_id: 0x%X remote_id: 0x%X ",_connection->interface()->name(),_self_id,_remote_id);
...
bool ULSBusTransaction::frameBufferCheck()
{
    ...
    if(_buf->isBufferComlite())
    {
        _buf->setToObject();
        switch(_state){
        case ULSBUST_BOI_RECEIVE_F: // No ack

            break;
        case ULSBUST_AOI_RECEIVE_F: // Ack anyway we are reading requested object - no permittion check;
            ...
            break;
        case ULSBUST_RWOI_RECEIVE_F: // Ack only if we receiving object
            if(_buf->obj()){
                if(_buf->obj()->permition() == ULSBUS_OBJECT_PERMITION_READONLY)
                {
                    _connection->sendAck(ULSBUS_ACK_OBJECT_ACCESS_DENIED,_cmd,_self_id,_remote_id);
                    ULSBUS_LOG("%s: Sending ULSBUS_ACK_OBJECT_ACCESS_DENIED : self_id: 0x%X remote_id: 0x%X ",_connection->interface()->name(),_self_id,_remote_id);
                    close();
                    return false;
                }
...
bool ULSBusTransaction::processPacket()
{
    ...
    case ULSBUST_TRANSMIT_COMPLITE_WAIT_ACK:
        if((pxRxPack->ack.ackcmd >> 5) == ULSBUS_ACK_COMPLITE){
            ...
            if(_buf->obj())
            {
                _buf->obj()->state(ULSBUS_OBJECT_STATE_OK);
            }else{
